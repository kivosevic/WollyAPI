...
public interface WalletItemRepository extends JpaRepository<WalletItem, UUID> {
    ...
    @Query(nativeQuery = true, value = "select sum(amount),w.cryptocurrency_id,c.icon,c.name,c.abbreviation,c.value_of_one "
            + " from wallet_item w "
            + " inner join cryptocurrency c "
            + " on w.cryptocurrency_id= c.id "
            + " where w.wallet_uuid=:uuid"
            + " group by w.cryptocurrency_id,c.icon,c.name,c.abbreviation,c.value_of_one;")
